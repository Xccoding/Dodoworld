...
LinkLuaModifier("modifier_mine_driller_dash_passive", "units/abilities/dungeon/kobold_mine_cave/mine_driller_dash.lua", LUA_MODIFIER_MOTION_BOTH)
--Abilities
...
function mine_driller_dash:GetIntrinsicModifierName()
    return "modifier_mine_driller_dash_passive"
end
---------------------------------------------------------------------
--Modifiers
...
--=======================================modifier_mine_driller_dash_passive=======================================
if modifier_mine_driller_dash_passive == nil then
    modifier_mine_driller_dash_passive = class({})
end
function modifier_mine_driller_dash_passive:IsHidden()
    return true
end
function modifier_mine_driller_dash_passive:IsDebuff()
    return false
end
function modifier_mine_driller_dash_passive:IsPurgable()
    return false
end
function modifier_mine_driller_dash_passive:IsPurgeException()
    return false
end
function modifier_mine_driller_dash_passive:OnCreated(params)
    if IsServer() then
        self:StartIntervalThink(1)
    end
end
function modifier_mine_driller_dash_passive:OnRefresh(params)
end
function modifier_mine_driller_dash_passive:OnDestroy(params)
end
function modifier_mine_driller_dash_passive:DeclareFunctions()
    return {
    }
end
function modifier_mine_driller_dash_passive:CDeclareFunctions()
    return {
    }
end
function modifier_mine_driller_dash_passive:OnIntervalThink()
    local hAbility = self:GetAbility()
    local hParent = self:GetParent()
    local enemies =  FindUnitsInLine(hParent:GetTeamNumber(), hParent:GetAbsOrigin(), hParent:GetAbsOrigin() + Vector(0, -1, 0) * 3000, nil, 1000, hAbility:GetAbilityTargetTeam(), hAbility:GetAbilityTargetType(), hAbility:GetAbilityTargetFlags())

    if hAbility:IsCooldownReady() and #enemies > 0 then
